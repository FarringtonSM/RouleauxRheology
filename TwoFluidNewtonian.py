...
# sheets = ['shear 0.1 1_s','shear 0.2 1_s','shear 0.35 1_s','shear 0.7 1_s',
#           'shear 1.0 1_s','shear 2.0 1_s']
# ii = [39,39,39,39,39,199]
...
with pd.ExcelWriter(file1) as writer:
    for i in range(6):
        ...
        for stress in df['Stress'].to_numpy():
            # stress_end = df['Stress'].iloc[-10:-1].mean()
            # stress_end = df['Stress'].iloc[ii[i]]
            # visc_b = cs(stress)
            # visc_b = HB_viscosity(stress,tau_0,K,n)
            bnds = ((0.0,None),)
            x0 = (0.01)
            ...
            visc_w = 0.00375 # Viscosity of dextran medium (Pa.s)
...
#     visc_b = cs(stress_end)
#     # visc_w = 0.001 # Viscosity of water (Pa.s)
#     visc_w = 0.00375 # Viscosity of dextran medium (Pa.s)
    
#     root = fsolve(fun,0,args=(visc_b,visc_w,shear_rate_SS[i],stress_end))
    
#     delta = root*L
#     deltas = np.append(deltas,delta)
   
#     shear_b = stress_end/visc_b
#     shear_bs = np.append(shear_bs,shear_b)
    
#     shear_w = stress_end/visc_w
#     shear_ws = np.append(shear_ws,shear_w)
    
# df = pd.DataFrame({'ShearRate_1_s':shear_rate_SS,
#                     'Deltas_um':deltas,
#                     'Shear_b_invS':shear_bs,
#                     'Shear_w_invS':shear_ws})
# file2 = path + 'DeltaVsShearRate.xlsx'
# df.to_excel(file2,index=False)

#%% Cell-free layer at characteristic time
# deltas = np.array([])
# shear_bs = np.array([])
# shear_ws = np.array([])
# a_s = np.array([])
...
#     # inflection_points = get_inflection_points(df['Step time'].to_numpy(), df['Stress'].to_numpy())
#     I_max = df['Stress'].idxmax()
#     I_min = df['Stress'].iloc[I_max:-1].idxmin()
    
#     time = df['Step time'].iloc[I_max:I_min] - df['Step time'].iloc[I_max]
#     stress = df['Stress'].iloc[I_max:I_min]
    
#     def Fcost(x,time,stress):
#         a = x[0]
#         b = x[1]
#         # beta = x[2]
#         c = x[2]
        
#         stress_pred = stretched_exp(time,a,b,c)
        
#         return np.sqrt(np.mean((stress-stress_pred)**2))
    
#     x0 = [1,1,1]
#     sol = opt.minimize(Fcost,x0,(time,stress))
...
#     tau = b + df['Step time'].iloc[I_max]
    
#     fig, ax = plt.subplots()
#     y = stretched_exp(time,a,b,c)
#     ax.plot(time,y)
#     ax.plot(time,stress,'o')
#     plt.show()

#     a_s = np.append(a_s,a)
#     b_s = np.append(b_s,b)
#     # beta_s = np.append(beta_s,beta)
#     c_s = np.append(c_s,c)
#     tau_s = np.append(tau_s,tau)
    
#     # Find data point that is close in time to "b", the characteristic time, and use it for the delta analysis
#     abs_dif = np.abs(df['Step time']-(b+df['Step time'].iloc[I_max]))
#     closest_index = abs_dif.idxmin()
    
#     s = df['Stress'].iloc[closest_index]    
    
#     visc_b = cs(s)
#     # visc_w = 0.001 # Viscosity of water (Pa.s)
#     visc_w = 0.00375 # Viscosity of dextran medium (Pa.s)
    
#     root = fsolve(fun,0,args=(visc_b,visc_w,shear_rate_SS[i],s))
...
# df = pd.DataFrame({'ShearRate_1_s':shear_rate_SS[0:6],
#                     'Deltas_um':deltas,
#                     'Shear_b_invS':shear_bs,
#                     'Shear_w_invS':shear_ws,
#                     'a':a_s,
#                     'b':b_s,
#                     'c':c_s,
#                     'Characteristic Time':tau_s})
# file3 = path + 'DeltaVsCharacteristicTime.xlsx'
# df.to_excel(file3,index=False)
